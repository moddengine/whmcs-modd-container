...
func (c Config) Validate() error {
	...
	if (c.DNS.Endpoint == "") != (c.DNS.KeyFile == "") {
		return errors.New("dns.endpoint and dns.key_file must be configured together")
	}
	if c.DNS.Endpoint != "" {
		if endpoint, err := url.ParseRequestURI(c.DNS.Endpoint); err != nil || endpoint.Scheme != "http" && endpoint.Scheme != "https" || endpoint.Host == "" {
			return errors.New("dns.endpoint must be an HTTP(S) URL")
		}
		if !filepath.IsAbs(c.DNS.KeyFile) {
			return errors.New("dns.key_file must be absolute")
		}
	}
